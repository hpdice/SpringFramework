...
public class MainClassUseConfig {
	...
	public static void main(String[] args) {
		...
		/* 
		 * IOC 컨테이너 생성
		 * 동일한 스프링 컨테이너 이므로 ref 속성으로 다른 스프링 설정 파일의 빈을 참조할 수 있음
		*/
		// 에너테이션을 이용한 Java 파일로 스프링 설정 파일 변경 
		...
		// bean으로 생성하지 않음 
		// *** 스프링은 bean을 싱글턴으로 관리하기 때문에 개별 관리가 필요한 Student의 데이터 오염을 방지 
...
// ?		registerService = ctx.getBean("studentRegisterService",StudentRegisterService.class);
		...
		registerService.register(new Student("hbs006","deer","p0006","melissa",
				26,'w',"Music"));
		
		printStudentInformationService.printStudentsInfo(); // 업데이트 확인
		...
		modifyService.modify(new Student("hbs006","pig","p0006","melissa",27,
				'w',"Comnputer"));
